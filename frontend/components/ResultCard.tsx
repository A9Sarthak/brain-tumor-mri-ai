...
import { FileText, CheckCircle2, AlertTriangle, HelpCircle, Activity, ShieldCheck, Check, AlertOctagon } from 'lucide-react';
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-xs overflow-hidden flex flex-col h-full transition-all">
      ...
      <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        ...
        {prediction ? (
          prediction.is_withheld ? (
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-700 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-900/60 px-2 py-0.5 rounded-full">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        {!prediction ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6 min-h-[260px] rounded-xl border border-rose-200 dark:border-rose-900/60 bg-rose-50/30 dark:bg-rose-950/20 space-y-3">
            <div className="w-12 h-12 rounded-full bg-rose-100 dark:bg-rose-900/60 text-rose-600 dark:text-rose-400 flex items-center justify-center shadow-2xs border border-rose-200 dark:border-rose-800">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 max-w-xs mt-2 leading-relaxed">
                {prediction.withheld_reason || "This image appears substantially different from the MRI data used by this model. A reliable classification cannot be provided."}
              </p>
            </div>
            <div className="w-full pt-2">
              <button
                onClick={onOpenReport}
                className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-800 transition-colors"
              >
